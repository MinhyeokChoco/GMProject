class Post { // 클래스 생성
    constructor(number, title, content, _name, _date, views) { // 생성자 함수로 제목, 내용, 작성자, 작성일, 조회수 생성
        this.number = number // 글번호
        this.title = title // 제목
        this.content = content // 내용
        let localname = JSON.parse(localStorage.getItem("lUserInfor")); // 현재 접속중인 유저 로그인 정보
        this.id = localname.userId // 로그인 된 유저의 id
        this.name = localname.userNickname; // 로그인 된 유저의 닉네임
        this.date = _date // 작성일
...
function write() { // write라는 함수 생성
    ...
    writeBtn.onclick = () => { // 글쓰기 버튼 클릭 했을 때
        const csList = new Post(writeData.length, titleValue.value, contentValue.value, name, `${year}-${month}-${day}`, 0) // csList 변수에 제목, 내용, 작성자, 작성일, 조회수 할당
